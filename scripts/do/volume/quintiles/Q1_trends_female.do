...
preserve
	...
	*--------------------------------------------------------------------------
	* STEP 1: Calculate Q1 for females
	*--------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------
	* STEP 2: Calculate trend lines for Q1 females
	*--------------------------------------------------------------------------
	use `original_data', clear
	...
	*--------------------------------------------------------------------------
	twoway ///
		(rcap ci_upper ci_lower months_from_birth, lcolor(red%30)) ///
		(line normalized_tweets months_from_birth, lwidth(medthick) lcolor(red)) ///
		...
		, xline(0, lpattern(dash) lcolor(edkblue)) ///
		  ...
		  title("Q1 Female Tweeting Frequency with Trend Lines") ///
		  subtitle("(Lowest Volume Quintile, Normalized to Pre-Birth Baseline = 100%)") ///
		  legend(order(2 3) label(2 "Q1 Females") label(3 "Trend Lines") ///
				 position(2) ring(0)) ///
		  note("Note: Q1 = lowest 20% of female tweeters." ///
			   "Pre-trend: months -18 to -9. Post-trend: months 0 to 17." ///
			   "Sample: ~496 female accounts") ///
		  graphregion(color(white)) ///
		  xlabel(-18(2)18)
	graph export "$volume_figs/twt_behavior/quintiles/Q1_female_trendlines.jpg", replace
restore

*------------------------------------------------------------------------------
* COMPARISON 1: Pre (-18 to -9) vs Post (0 to 17)
* Excludes the immediate peri-birth period
*------------------------------------------------------------------------------
preserve
...
*------------------------------------------------------------------------------
* Q1 FEMALE ONLY with trend lines (-18 to -1, 0-17)
*------------------------------------------------------------------------------
preserve
	* Keep only females
	keep if female == 1
	...
	*--------------------------------------------------------------------------
	twoway ///
		...
		, xline(0, lpattern(dash) lcolor(edkblue)) ///
		  ...
		  graphregion(color(white)) ///
		  xlabel(-18(2)18)
	graph export "$volume_figs/twt_behavior/quintiles/Q1_female_pre_post.jpg", replace
restore
...
* Includes all data, birth month in post-period
*------------------------------------------------------------------------------
preserve
    * Keep only female accounts
    keep if female == 1
