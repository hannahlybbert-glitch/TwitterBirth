...
// Created: 01/26/2026
// Purpose: MALES --> Normalized Quintiles + trend lines

use "$volume_analysis/tweet_volume_analysis_sample.dta", clear

*------------------------------------------------------------------------------
* ALL 5 QUINTILES MALES with SINGLE OVERALL trend line
	...
	*--------------------------------------------------------------------------
	twoway ///
		...
		, xline(0, lpattern(dash) lcolor(black)) ///
		  ...
		  title("Male Tweeting Frequency by Quintile with Overall Trend Lines") ///
		  ...
		  note("Note: Each quintile normalized to its own pre-birth average." ///
			   "Trend lines based on all male data combined." ///
			   "Pre-trend: months -18 to -1. Post-trend: months 0 to 17." ///
			   "Sample: 3,190 male accounts") ///
		  graphregion(color(white)) ///
		  xlabel(-18(2)18)
	graph export "$volume_figs/twt_behavior/quintiles/male_Q_trends.jpg", replace
restore

*------------------------------------------------------------------------------
* REGRESSION TABLES: Pre, Post, and Interaction (All Males)
*------------------------------------------------------------------------------
preserve
    ...
    * Export to Word document
    esttab pre_birth post_birth interaction using "$volume_figs/twt_behavior/regressions/male_pre_post_reg.rtf", ///
        replace ///
        title("Male Tweeting Trends: Pre-Birth, Post-Birth, and Interaction Models") ///
	...
	*--------------------------------------------------------------------------
	twoway ///
		...
		, xline(0, lpattern(dash) lcolor(black)) ///
		  ...
		  graphregion(color(white)) ///
		  xlabel(-18(2)18)
	graph export "$volume_figs/twt_behavior/quintiles/male_Q_preg_trend.jpg", replace
restore



*------------------------------------------------------------------------------
* PREGNANCY REGRESSION TABLES: Pre, Post, and Interaction (All Males)
*------------------------------------------------------------------------------
preserve
    * Keep only male accounts
    keep if female == 0
    ...
    * Export to Word document
    esttab pre_birth post_birth interaction using "$volume_figs/twt_behavior/regressions/male_preg_reg.rtf", ///
        replace ///
        title("Male Tweeting Trends: Pre-Pregnancy, Post-Birth, and Interaction Models") ///
        mtitles("Pre-Birth" "Post-Birth" "Interaction") ///
        b(3) se(3) ///
        star(* 0.10 ** 0.05 *** 0.01) ///
        stats(N r2 r2_a, fmt(0 3 3) labels("Observations" "R-squared" "Adj. R-squared")) ///
