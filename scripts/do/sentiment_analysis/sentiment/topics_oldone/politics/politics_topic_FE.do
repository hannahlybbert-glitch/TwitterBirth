...
* Created: 12/01/2025
* Purpose: Individual Fixed effects on Political Orientedness
...
areg news__social_concern post_birth, absorb(author_id)
...
* ----- FAMILY SCORE ----- *
preserve
areg news__social_concern _*, absorb(author_id)
...
twoway ///
	...
	xline(0, lcolor(black)) xline(-1, lcolor(gs8) lpattern(dash)) ///
      ...
      ytitle("Estimated Effect on Political Orientation of Tweets") ///
      title("Fixed-Effects Estimates of Political Oriented Tweets") ///
      legend(label(1 "95% CI") label(2 "Coefficient") position(4) ring(0)) ///
	  note("Note: Birth announcement tweet excluded. Base Month = -1. Used <news__social_concern>")
graph export "$tweetNLP_figs/topic_class/fixed_effects/news_coeffs.jpg", replace
restore
...
preserve
	...
	areg news__social_concern _female*, absorb(author_id)
	parmest, norestore
...
preserve
	...
	areg news__social_concern _male*, absorb(author_id)
	parmest, norestore
...
twoway ///
    ...
    xtitle("Months from Birth") ///
      ytitle("Estimated Effect on Political Orientation of Tweets") ///
      title("Fixed-Effects Estimates of Political Oriented Tweets by Gender") ///
      legend(label(1 "95% CI") label(2 "Coefficient") position(4) ring(0)) ///
	  note("Note: Birth announcement tweet excluded. Base Month = -1. Used <news__social_concern>") ///
    legend(order(3 "Female" 4 "Male") position(2) ring(0))
	
graph export "$tweetNLP_figs/topic_class/fixed_effects/news_coeffs_bygender.jpg", replace
restore
...
twoway ///
	...
	xline(0, lcolor(black)) xline(-1, lcolor(gs8) lpattern(dash)) ///
      ...
      title("Fixed-Effects Estimates of Political Oriented Tweets") ///
      legend(label(1 "95% CI") label(2 "Coefficient") position(4) ring(0)) ///
	  note("Note: Tweets +/- 14 days from birth excluded. Base Month = -1. Used <news__social_concern>")
graph export "$tweetNLP_figs/topic_class/fixed_effects/noBM/news_coeffs_noBM.jpg", replace
restore
...
twoway ///
    ...
    xtitle("Months from Birth") ///
      ...
      legend(label(1 "95% CI") label(2 "Coefficient") position(4) ring(0)) ///
	  note("Note: Tweets +/- 14 days from birth excluded. Base Month = -1. Used <news__social_concern>") ///
    legend(order(3 "Female" 4 "Male") position(4) ring(0))
graph export "$tweetNLP_figs/topic_class/fixed_effects/noBM/news_coeffs_noBM_bygender.jpg", replace
restore