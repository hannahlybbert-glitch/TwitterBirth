...
* Created: 11/17/2025
* Purpose: Probability tweets for family, politics, and sports related around birth (simple and by gender)
	...
	* Standard error and 95% confidence intervals
	...
	gen ci_upper = family + 1.96 * se
		...
		* Average family rating pre/post birth
		sum family if months_from_birth < 0
...
twoway (rcap ci_upper ci_lower months_from_birth, lcolor(navy%50)) ///
    ...
    , xline(0, lpattern(dash) lcolor(edkblue)) ///
	  ...
	  legend(order(2 1 3 4) ///
		 ...
		 note("Note: Birth announcement tweet excluded. 95% CI. Red line = pre-birth average. Green line = post-birth average.") ///
...
twoway ///
    ...
    , ///
	  ...
	  note("Note: Birth announcement tweet excluded. 95% CI. Light lines = pre-birth average. Darker lines = post-birth average.") ///
      graphregion(color(white)) ///
...
preserve
	...
	* Standard error and 95% confidence intervals
	...
	gen ci_upper = news__social_concern + 1.96 * se
		...
		* Average family rating pre/post birth
		sum news__social_concern if months_from_birth < 0
		local pre_birth_avg = r(mean)
		sum news__social_concern if months_from_birth >= 0
		local post_birth_avg = r(mean)
...
twoway (rcap ci_upper ci_lower months_from_birth, lcolor(navy%50)) ///
    ...
    , xline(0, lpattern(dash) lcolor(edkblue)) ///
	  ...
	  legend(order(2 1 3 4) ///
		 ...
		 note("Note: Birth announcement tweet excluded. Red line = pre-birth average. Green line = post-birth average. 'News & Social Concern probability.", size(vsmall)) ///
         graphregion(color(white)) ///
         xlabel(-18(2)18)
* Save graph
...
preserve
	...
	* Compute SE and 95% CI
	...
	gen ci_upper = news__social_concern + 1.96 * se
	* Female averages
	sum news__social_concern if female==1 & months_from_birth < 0
	local fem_pre_avg = r(mean)
	sum news__social_concern if female==1 & months_from_birth >= 0
	local fem_post_avg = r(mean)
	* Male averages
	sum news__social_concern if female==0 & months_from_birth < 0
	local male_pre_avg = r(mean)
	sum news__social_concern if female==0 & months_from_birth >= 0
	local male_post_avg = r(mean)
...
preserve
	...
	* Standard error and 95% confidence intervals
	...
	gen ci_upper = sports + 1.96 * se
		...
		* Average family rating pre/post birth
		sum sports if months_from_birth < 0
		local pre_birth_avg = r(mean)
		sum sports if months_from_birth >= 0
		local post_birth_avg = r(mean)
...
twoway (rcap ci_upper ci_lower months_from_birth, lcolor(navy%50)) ///
    ...
    , xline(0, lpattern(dash) lcolor(edkblue)) ///
	  ...
	  legend(order(2 1 3 4) ///
		 ...
		 note("Note: Birth announcement tweet excluded. Red line = pre-birth average. Green line = post-birth average. 'Sports probability.", size(vsmall)) ///
         graphregion(color(white)) ///
         xlabel(-18(2)18)
* Save graph
...
preserve
	...
	* Compute SE and 95% CI
	...
	gen ci_upper = sports + 1.96 * se
	* Female averages
	sum sports if female==1 & months_from_birth < 0
	local fem_pre_avg = r(mean)
	sum sports if female==1 & months_from_birth >= 0
	local fem_post_avg = r(mean)
	* Male averages
	sum sports if female==0 & months_from_birth < 0
	local male_pre_avg = r(mean)
	sum sports if female==0 & months_from_birth >= 0
	local male_post_avg = r(mean)
...
twoway ///
    ...
    , ///
      ...
      xlabel(-18(2)18) ///
		 ...
		 note("Note: Birth announcement tweet excluded. Light line = pre-birth average. Darker line = post-birth average. 'Sports' probability.", size(vsmall)) ///
      graphregion(color(white)) ///
      legend(order(2 "Female Avg"  ///
                   4 "Male Avg") ///
             position(3) ring(0))
