...
* Created: 01/13/2025
* Purpose: Individual Fixed effects on Sports Orientedness
...
areg sports post_birth, absorb(author_id)
...
* ----- Sports SCORE ----- *
preserve
areg sports _*, absorb(author_id)
...
twoway ///
	...
	xline(0, lcolor(black)) ///
      ...
      ytitle("Estimated Effect on Sports Orientation of Tweets") ///
      title("FE Estimates of Sports-Related Tweet Probability") ///
...
graph export "$sports_figs/fixed_effects/sports_coeffs.jpg", replace
restore
...
preserve
	...
	areg sports _female*, absorb(author_id)
	parmest, norestore
...
preserve
	...
	areg sports _male*, absorb(author_id)
	parmest, norestore
...
twoway ///
    ...
    xtitle("Months from Birth") ///
      ytitle("Estimated Effect on Sports Orientation of Tweets") ///
      title("FE Estimates of Sports-Related Tweet Probability by Gender") ///
...
graph export "$sports_figs/fixed_effects/sports_coeffs_bygender.jpg", replace
restore
...
twoway ///
	...
	xline(0, lcolor(black)) xline(-1, lcolor(gs8) lpattern(dash)) ///
      ...
      xtitle("Months from Birth") ///
      ytitle("Estimated Effect on Sports Orientation of Tweets") ///
      title("Fixed-Effects Estimates of Sports Oriented Tweets") ///
      legend(label(1 "95% CI") label(2 "Coefficient") position(4) ring(0)) ///
	  note("Note: Tweets +/- 14 days from birth excluded. Base Month = -1.")
graph export "$tweetNLP_figs/topic_class/fixed_effects/noBM/sports_coeffs_noBM.jpg", replace
restore
...
twoway ///
    ...
    xtitle("Months from Birth") ///
      ytitle("Estimated Effect on Sports Orientation of Tweets") ///
      title("Fixed-Effects Estimates of Sports Oriented Tweets by Gender") ///
      legend(label(1 "95% CI") label(2 "Coefficient") position(4) ring(0)) ///
	  note("Note: Tweets +/- 14 days from birth excluded. Base Month = -1.") ///
    legend(order(3 "Female" 4 "Male") position(2) ring(0))
graph export "$tweetNLP_figs/topic_class/fixed_effects/noBM/sports_coeffs_noBM_bygender.jpg", replace
restore