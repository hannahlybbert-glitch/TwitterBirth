...
* Created: 02/20/2026
* Purpose: Fixed effects estimates of tweet sentiment — overall and by gender
...
preserve
	...
	* Insert baseline observation at month -1 so connected line passes through (-1, 0)
	...
	twoway ///
		...
		$xlab_months ///
		$xtitle_months ///
		ytitle("Sentiment", size($ytitle_size)) ///
		$ylab ///
		$leg_off ///
		$region

	graph export "$sent_out/sentiment_fe.$fig_format", replace
restore


* ----------------------------------------------------------------
* FIGURE 2: Sentiment by gender
* ----------------------------------------------------------------
...
preserve
	...
	* Insert baseline observation at month -1 so connected line passes through (-1, 0)
	...
	graph export "$sent_out/sentiment_fe_gender.$fig_format", replace
restore
