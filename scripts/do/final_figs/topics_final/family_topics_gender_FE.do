...
* Created: 02/20/2026
* Purpose: Fixed effects estimates of family topic probability — overall and by gender
...
preserve
	...
	* Insert baseline observation at month -1 so connected line passes through (−1, 0)
	...
	twoway ///
		...
		$xlab_months ///
		$xtitle_months ///
		ytitle("Percentage Family Related", size($ytitle_size)) ///
		$ylab ///
		$leg_off ///
		$region

	graph export "$topics_out/family_topics_FE.$fig_format", replace
restore
...
preserve
	...
	* Insert baseline observation at month -1 so connected line passes through (−1, 0)
	...
	graph export "$topics_out/family_topics_gender_FE.$fig_format", replace
restore
