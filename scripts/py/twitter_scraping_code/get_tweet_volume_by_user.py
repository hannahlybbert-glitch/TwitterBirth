...
# Load environment variables (twitter API bearer token)
...
OUTPUT_CSV = f"data/raw/tweet_volume_by_user_{year}.csv"
...
# create paths for the user info files
# 2018 file will need to be run through grab_missing_user_info.py to get bio and follower count, other years of data are fine
if year != "2018":
    USER_PATH = f"data/raw/user_info_{year}.csv"
else:
    USER_PATH = f"data/raw/user_info_{year}_missing_bio.csv"

# save "user_info" file
df.to_csv(USER_PATH, encoding = "utf-8", index = False)
print(f"User file saved as {USER_PATH}.")
...
# Filter out already processed birth events
new_births = unique_births[~unique_births["unique_id"].isin(processed_user_ids)]
...
def get_tweet_counts(user_id, start_time, end_time):
    """Fetches daily original and quote tweet counts for a user within the given time range."""
    query_params = {
    ...
    while True:
        try:
            ...
            #     time.sleep(15 * 60)  # 15-minute sleep
                ...
                print(f"[{sleep_start.strftime('%H:%M:%S')}] Rate limit reached! Sleeping for 15 minutes...")
                time.sleep(15 * 60)
                print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Resuming after rate limit sleep.")
...
def save_to_csv(data, unique_id, filename):
    ...
    with open(filename, mode="a", newline="", encoding="utf-8") as file:
        ...
        if data:
            ...
            # If there is no tweet data, write a zero row
            writer.writerow([
                unique_id,
                user_id,
                user_created_at,
...
def main():
    """Fetch tweet counts for each unprocessed birth event and append results to CSV."""
    if new_births.empty:
    ...
    rate_limit_count = 0
    total_events = len(new_births)

    for i, (_, row) in enumerate(tqdm(new_births.iterrows(), total=total_events, desc="Processing Events", unit="event")):
        ...
        avg_time_per_event = elapsed / (i + 1)
        est_time_remaining = avg_time_per_event * (total_events - (i + 1))
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Processed {i+1}/{total_events} — "
              f"Elapsed: {elapsed:.1f}s — ETA: {est_time_remaining/60:.1f} min")

    print(f"\nFinished fetching tweet counts. Results saved to {OUTPUT_CSV}")
    print(f"Total time elapsed: {time.time() - start_time_script:.1f} seconds")
